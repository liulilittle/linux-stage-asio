...
template<>
class GenericResponse<CreateOrderResponse> {
private:
    int Code;
    const char* Message;
    unsigned short Message_len;
    CreateOrderResponse* Tag;
...
public:
    ...
    CreateOrderResponse* GetTag() {
        return this->Tag;
    }
    CreateOrderResponse* MutableTag() {
        if (this->Tag == NULL) this->Tag = new CreateOrderResponse();
        return this->Tag;
    }
...
public:
    ...
    void SetTag(CreateOrderResponse* value) {
        CreateOrderResponse* Tag_ptr = this->Tag;
...
public:
    bool Deserialize(unsigned char*& buf, int& count) {
        ...
        if (*buf != 0) {
            ...
            if (NULL == this->Tag) this->Tag = new CreateOrderResponse();
